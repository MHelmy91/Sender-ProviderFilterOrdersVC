...
class ProviderFilterOrdersVC: UIViewController {
    ...
    @objc func btnShowDateTab(sender: UIButton) {
...
extension ProviderFilterOrdersVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func setTableview() {
        ...
        let identfireHeader = String(describing: ProviderHeaderTableviewFilterOrders.self)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            if paymentMethod[indexPath.row] == filterOrders.paymentMethod {
                cell.setViewLayer()
            } else {
                cell.reSetViewLayer()
            }
        ...
        if status[indexPath.row] == filterOrders.status {
            cell.setViewLayer()
        } else {
            cell.reSetViewLayer()
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       tableView.deselectRow(at: indexPath, animated: true)
        ...
        if indexPath.section == 0 {
            filterOrders.paymentMethod = paymentMethod[indexPath.row]
        } else {
            filterOrders.status = status[indexPath.row]
        }
        tableview.reloadData()
   }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let identfire = String(describing: ProviderHeaderTableviewFilterOrders.self)
        let headerView =  tableView
            .dequeueReusableHeaderFooterView(withIdentifier: identfire) as! ProviderHeaderTableviewFilterOrders
...
extension ProviderFilterOrdersVC {
    ...
    @objc func addDateToTextField(){
        ...
        searchTextfieldOutlet.text = date
...
extension ProviderFilterOrdersVC {
    ...
    func setBtnShowDate() {
        ...
        let btnShowDate = UIButton()
        btnShowDate.addTarget(self, action: #selector(btnShowDateTab(sender:)), for: .touchUpInside)
        let image = UIImage(named: "Iconly-Light-Calendar")
        btnShowDate.setTitle("", for: .normal)
        btnShowDate.setImage(image, for: .normal)
        btnShowDate.widthAnchor.constraint(equalToConstant: 14).isActive = true
        btnShowDate.heightAnchor.constraint(equalToConstant: 16).isActive = true
        searchTextfieldOutlet.rightViewMode = .always
        searchTextfieldOutlet.rightView = btnShowDate
    }
